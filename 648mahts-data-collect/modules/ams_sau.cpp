...
const char* ams_sau_str[]={
	"status",
	"device_offline",
};
	
static const char* productCode = "ams_sau";
...
void AMS_SAU::init(int devId,BlockQueue<ams_sau> &bq,modbus *mb,mutex *mux)
{
	m_mb_plc1 = mb;
	m_mux_plc1 = mux;

	m_devId = devId + 1;  //TBD, 2, 3
	
	m_clientId.clear();
	m_clientId.append(productCode);
	m_clientId.append("_device");
	m_clientId.append(to_string(m_devId));
...
// 连接丢失回调函数
void AMS_SAU::connectionLostCallback(void* context, char* cause) 
{
	// 处理连接丢失事件
	CallbackContext* ctx = static_cast<CallbackContext*>(context);
	AMS_SAU* client = static_cast<AMS_SAU*>(ctx->manager);
...
// 消息到达回调函数
int AMS_SAU::messageArrivedCallback(void* context, char* topicName, int topicLen,MQTTClient_message* message) 
{
	// 处理接收到的消息
	CallbackContext* ctx = static_cast<CallbackContext*>(context);
	AMS_SAU* client = static_cast<AMS_SAU*>(ctx->manager);
...
cout<<"payload:"<<payloadptr<<endl;
	...
	if(0 == client->m_subs[0].compare(topicName))
	{
		...
		for(int i = 0; i < client->m_property_size;i++)
			client->m_property[ams_sau_str[i]]=i;
...
// 交付完成回调函数
void AMS_SAU::deliveryCompleteCallback(void* context, MQTTClient_deliveryToken dt) 
{
	// 处理交付完成事件
	//std::cout << "Message delivery complete." << std::endl;
}

void AMS_SAU::subClient()
{
    int rc;
    int ch;
...
void AMS_SAU::pubClient(BlockQueue<ams_sau>& bq)
{
	while(1)
	{
...
void AMS_SAU::start(BlockQueue<ams_sau> &bq)
{
	m_sub = new thread(&AMS_SAU::subClient,this);
	m_sub->detach();

	m_pub = new thread(&AMS_SAU::pubClient,this,ref(bq));
	m_pub->detach();

	m_thr_reconn = new thread(&AMS_SAU::reconn,this);
	m_thr_reconn->detach();

	m_thr_resend = new thread(&AMS_SAU::resnd,this);
	m_thr_resend->detach();
...
string AMS_SAU::property_reply(char flag,const ams_sau& one,const string& msg_id)
{
	...
	{
		switch(m_v_index[i]){
			case 0:writer.Int(one.status);break;
			case 1:writer.Int(one.device_offline);break;
