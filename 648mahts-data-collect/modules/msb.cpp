...
#else
  "hg_start_failure",
"hg_acb_abnormal_trip",
"hg_rve_power",
"hg_over_current",
"hg_acb_close_fail",
"synchro_failure",
"ac440v_bus_a_insulation_low440v",
"ac440v_bus_b_insulation_low440v",
"ac220v_bus_a_insulation_low440v",
"ac220v_bus_b_insulation_low440v",
"pre_trippt",
"dc24v_source_fault",
"pt_ec_circuit_source_failure",
"ac440v_bus_tie_cb_non_close",
"uneven_load_sharing",
"msb_es_line_break",
"msb_common_alarm",
"busbar_a_source_alive",
"busbar_b_source_alive",
"hg_acb_close",
"dt1secondary_acb_closed440v",
"dt2secondary_acb_closed440v",
"ac440v_bus_tie_breaker_close",
"ac220v_bus_tie_breaker_close",
"shore_cb_closed440v",
"lt1primary_acb_close",
"lt1secondary_acb_close",
"lt2primary_acb_close",
"lt2secondary_acb_close",
"hg_running",
"eg_running",
"hg_current_l1a",
"hg_current_l2a",
...
"hg_voltage_l2_l3v",
"hg_frequencyhz",
"hg_reactive_powerkvar",
"hg_power_factorcos_phi",
"hg_powerkw",
//"esb_ac415v_bus_low_insulation",
"esb_ac440v_bus_low_insulation",
"esb_ac220v_bus_low_insulation",
"esb_dc24v_power_failure",
"em_cy_generator_overload",
"esb_common_alarm",
"e_g_cb_abnormal_trip",
"ac440v_pms_fault",
"ac440v_dc24_power_fault",
"t3_cb_status",
"t4_cb_status",
"sc_cb_status",
"spare",
...
static const char* productCode = "msb440v";
...
void MSB::init(int devId,BlockQueue<msb> &bq,modbus *mb,mutex *mux)
{
	m_mb_plc1 = mb;
	m_mux_plc1 = mux;
...
// 连接丢失回调函数
void MSB::connectionLostCallback(void* context, char* cause) 
{
	// 处理连接丢失事件
	CallbackContext* ctx = static_cast<CallbackContext*>(context);
	MSB* client = static_cast<MSB*>(ctx->manager);
...
// 消息到达回调函数
int MSB::messageArrivedCallback(void* context, char* topicName, int topicLen,MQTTClient_message* message) 
{
	// 处理接收到的消息
	CallbackContext* ctx = static_cast<CallbackContext*>(context);
	MSB* client = static_cast<MSB*>(ctx->manager);
...
// 交付完成回调函数
void MSB::deliveryCompleteCallback(void* context, MQTTClient_deliveryToken dt) 
{
	// 处理交付完成事件
...
void MSB::pubClient(BlockQueue<msb>& bq)
{
	#if 1 
	while(1)
	{
...
void MSB::start(BlockQueue<msb> &bq)
{
	m_sub = new thread(&MSB::subClient,this);
	m_sub->detach();

	m_pub = new thread(&MSB::pubClient,this,ref(bq));
	m_pub->detach();

	m_thr_reconn = new thread(&MSB::reconn,this);
	m_thr_reconn->detach();

	m_thr_resend = new thread(&MSB::resnd,this);
	m_thr_resend->detach();

	m_thr_chksensor = new thread(&MSB::chksensor,this);
	m_thr_chksensor->detach();

//	thread *thr_range = new thread(&MSB::thSetRange,this);
//	thr_range->detach();

	thread *thr_threshold = new thread(&MSB::thSetThreshold,this);
	thr_threshold->detach();

	thread *thr_inhibit = new thread(&MSB::thSetInhibit,this);
	thr_inhibit->detach();

	thread *thr_delay = new thread(&MSB::thSetDelay,this);
	thr_delay->detach();
...
string MSB::property_reply(char flag,const msb& one,const string& msg_id)
{
...
#else
        case 0:writer.Int(one.hg_start_failure);break;
        case 1:writer.Int(one.hg_acb_abnormal_trip);break;
        case 2:writer.Int(one.hg_rve_power);break;
        case 3:writer.Int(one.hg_over_current);break;
        case 4:writer.Int(one.hg_acb_close_fail);break;
        case 5:writer.Int(one.synchro_failure);break;
        case 6:writer.Int(one.ac440v_bus_a_insulation_low440v);break;
        case 7:writer.Int(one.ac440v_bus_b_insulation_low440v);break;
        case 8:writer.Int(one.ac220v_bus_a_insulation_low440v);break;
        case 9:writer.Int(one.ac220v_bus_b_insulation_low440v);break;
        case 10:writer.Int(one.pre_trippt);break;
        case 11:writer.Int(one.dc24v_source_fault);break;
        case 12:writer.Int(one.pt_ec_circuit_source_failure);break;
        case 13:writer.Int(one.ac440v_bus_tie_cb_non_close);break;
        case 14:writer.Int(one.uneven_load_sharing);break;
        case 15:writer.Int(one.msb_es_line_break);break;
        case 16:writer.Int(one.msb_common_alarm);break;
        case 17:writer.Int(one.busbar_a_source_alive);break;
        case 18:writer.Int(one.busbar_b_source_alive);break;
        case 19:writer.Int(one.hg_acb_close);break;
        case 20:writer.Int(one.dt1secondary_acb_closed440v);break;
        case 21:writer.Int(one.dt2secondary_acb_closed440v);break;
        case 22:writer.Int(one.ac440v_bus_tie_breaker_close);break;
        case 23:writer.Int(one.ac220v_bus_tie_breaker_close);break;
        case 24:writer.Int(one.shore_cb_closed440v);break;
        case 25:writer.Int(one.lt1primary_acb_close);break;
        case 26:writer.Int(one.lt1secondary_acb_close);break;
        case 27:writer.Int(one.lt2primary_acb_close);break;
        case 28:writer.Int(one.lt2secondary_acb_close);break;
        case 29:writer.Int(one.hg_running);break;
        case 30:writer.Int(one.eg_running);break;
        case 31:writer.Double(one.hg_current_l1a);break;
        case 32:writer.Double(one.hg_current_l2a);break;
        case 33:writer.Double(one.hg_current_l3a);break;
        case 34:writer.Double(one.hg_voltage_l1_l3v);break;
        case 35:writer.Double(one.hg_voltage_l1_l2v);break;
        case 36:writer.Double(one.hg_voltage_l2_l3v);break;
        case 37:writer.Double(one.hg_frequencyhz);break;
        case 38:writer.Double(one.hg_reactive_powerkvar);break;
        case 39:writer.Double(one.hg_power_factorcos_phi);break;
        case 40:writer.Double(one.hg_powerkw);break;
        case 41:writer.Int(one.esb_ac415v_bus_low_insulation);break;
        case 42:writer.Int(one.esb_ac220v_bus_low_insulation);break;
        case 43:writer.Int(one.esb_dc24v_power_failure);break;
        case 44:writer.Int(one.em_cy_generator_overload);break;
        case 45:writer.Int(one.esb_common_alarm);break;
        case 46:writer.Int(one.e_g_cb_abnormal_trip);break;
        case 47:writer.Int(one.ac440v_pms_fault);break;
        case 48:writer.Int(one.ac440v_dc24_power_fault);break;
        case 49:writer.Int(one.t3_cb_status);break;
        case 50:writer.Int(one.t4_cb_status);break;
        case 51:writer.Int(one.sc_cb_status);break;
        case 52:writer.Int(one.spare);break;
        case 53:writer.Int(one.device_offline);break;
#endif
...
void MSB::init_mb_addr()
{
	int addr_ai = 7800;
	for(int i = 0; i < 70; i++)
		m_mp_delay_addr[msb_str[i]] = addr_ai + i;
	...
	for(int dg = 0; dg < 6; dg++)
	{
		addr_ai = 7870 + 20 * dg;
		for(int i = 1; i < 4; i++)
			m_mp_delay_addr[msb_str[70 + 8 * dg + i]] = addr_ai + i;

		m_mp_delay_addr[msb_str[70 + 8 * dg + 7]] = addr_ai + 5;

		for(int i = 6; i < 10; i++)
			m_mp_delay_addr[msb_str[70 + i]] = addr_ai + i;
	}
	...
	{
		for(int i = 0; i < 16; i++)
			m_mp_inhibit_addr[msb_str[16*ix + i]] = i + 100 * ix;
	}
	for(int i = 0; i < 6; i++)
		m_mp_inhibit_addr[msb_str[64 + i]] = i + 400;


	for(int dg = 0; dg < 6; dg++)
	{
		for(int i = 1; i < 4; i++)
			m_mp_inhibit_addr[msb_str[70 + 8 * dg + i]] =  i + 100 * dg + 500;
		
		m_mp_inhibit_addr[msb_str[70 + 8 * dg + 7]] = 100 * dg + 505;
		
		for(int i = 6; i < 10; i++)
			m_mp_inhibit_addr[msb_str[70 + i]] = 100 * dg + i + 500;;
	}
